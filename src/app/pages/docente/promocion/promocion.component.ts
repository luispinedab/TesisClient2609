...
export class PromocionComponent implements OnInit {
  ...
  settings = {
    ...
    columns: {
      ...
      IDAspirante: {
        title: 'Estado de Promoción',
        type: 'string',
        editable:false,
        filter:false,
        valuePrepareFunction: (data) => {
          return data.statusProm;
        }
      },
      set: {
        title: '',
  ...
  acomodar(arreglo:any){
    ...
    arreglo.forEach(element => {
      var objeto={
        ...
        if(element.IDAspirante.statusProm==0){
          element.IDAspirante.statusProm= "Reprobó";
        }
        else if (element.IDAspirante.statusProm==1){
          element.IDAspirante.statusProm= "Aprobó";
        }
    });
  }
  onUserRowSelect (event) {
    ...
    { var aspirantesactualizar=[];
      ...
      this.estudiantesselected.forEach(seleccion => {
        if(seleccion.IDAspirante.statusProm=="Reprobó")
        {
          aspirantes0.push(seleccion.IDAspirante.IDAspirantes)
        }
        else if(seleccion.IDAspirante.statusProm=="Aprobó")
        {
      ...
      this.aspirantesservice.updateApirantesProm(aspirantesactualizar).subscribe(res=>{
        console.log(res);
            this.getinfo();
        this.source.refresh();
      })
